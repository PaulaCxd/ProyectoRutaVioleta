...
namespace EncuestaRutaVioleta
{
    public partial class VinculoDetalle : Form
    {
        ...
        private Entidades.Vinculo Vinculo;
        private string Cuales;
        private Entidades.ViolenciaPsicologica TipoViolenciaPsicologica;
        ...
        {    
            ...
            var vinculoPersonaV = new List<Entidades.Vinculo>();
            var tipoViolenciaPsicologica = new List<Entidades.ViolenciaPsicologica>();
            for (int x = 0; x < clbVinculoPersonaV.CheckedItems.Count; x++)
            {
                vinculoPersonaV.Add(clbVinculoPersonaV.CheckedItems[x] as Entidades.Vinculo);
            }
            for (int x = 0; x < clbTipoViolenciaPsicologica.CheckedItems.Count; x++)
            {
                tipoViolenciaPsicologica.Add(clbTipoViolenciaPsicologica.CheckedItems[x] as Entidades.ViolenciaPsicologica);
            }

            DatosGenerales.RutaVioleta.ViolenciaPsicologica = tipoViolenciaPsicologica.FirstOrDefault();
            DatosGenerales.RutaVioleta.Vinculo = vinculoPersonaV.FirstOrDefault();
            var vinculoDetalles = new VinculoDetalle();
